...
- 매주 **월, 화 20시~22시 웹엑스로 진행**, 그 외에는 자율적으로 모각코
- **평일 모임 20:00-22:00**
- **주말은 시간 구애 없는 자율적으로 문제 풀기**
  - **주말간** 문제를 **1~2개씩** 선정해오기(가져오는 유형 범위는 누적 -> 복습 차원)
  - 팀원들이 골라온 문제들을 주중에 최대한 풀기 🤙
  - [백준 그룹](https://www.acmicpc.net/group/13731) 문제집으로 발행, 실시간으로 체크 가능

- 3월 수정 사항
  - 월요일 1대 다 리뷰 진행 [리뷰 방식](#code-review-제안사항)  
  - 화요일 Semi 모의 코테 진행 문제 출처: [코딩테스트 대비 문제집 with Baekjoon](https://github.com/tony9402/baekjoon/blob/main/picked.md)  
    당일 추천 문제 4문제 중 2문제 이상 1시간 반동안 풀기
...
5주차: 2022-03-07 ~ 2022-03-13 자율
  ...
  | 문제출처 | 문제번호 이름      | 문제유형    | 난이도 |
  | -------- | ----------------- | ----------- | -------- |
  |BOJ| [10830 행렬 제곱](https://www.acmicpc.net/problem/10830) | 분할정복 | G4 |
  |BOJ| [9465 스티커](https://www.acmicpc.net/problem/9465) | DP | S1 |
  |BOJ| [4803 트리](https://www.acmicpc.net/problem/4803) | 트리 | G4 |
...
### Code Review 제안사항
1. 로직적인 측면에서 시간복잡도, 공간복잡도 분석 해주기? → 추가 재고도 없는데, arrayList 쓰는게 낫지 않겠냐와 같은 
2. 못푼 문제가 있으면 어디까지 못풀었는지가 있을텐데, 왜 못풀었는지, 여기서 어떻게 해야 풀 수 있는지
3. 하드 코딩 섞어서 푼문제 리팩토링 제안
4. 풀고 이게 맞나 싶어서 다른 사람꺼 복붙한 경우는 자신의 풀이와 다른 사람 참고해서 바꾼 풀이를 가져와서 발표하기
5. 어떻게 생각해서 접근했는지가 중요(문제를 어떻게 해석, 키워드, 이렇게 접근했는데, 맞았다.) → 주석으로 적어놔서 공유하기
6. 똑같은 로직이어도 사람들마다 구현 방식이 달라서 깔끔하게 짠 사람 코드 보면 좋음 

→ 각자 코드(물어보고 싶은 코드나, 잘 짰다고 생각되는 코드)를 하나 가지고 와서 1대 다 리뷰 해보기
...
- [코딩테스트 대비 문제집 with Baekjoon](https://github.com/tony9402/baekjoon/blob/main/picked.md)
